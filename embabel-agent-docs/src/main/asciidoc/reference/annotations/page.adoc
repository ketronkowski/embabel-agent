...
The `@Trigger` annotation enables reactive behavior where an action only fires when a specific parameter type is the _most recently added_ value to the blackboard.
...
<1> The `@Trigger` annotation means this action only fires when `UserMessage` is the last result added to the blackboard.
...
With `@Trigger`, the annotated parameter must additionally be the most recent value added.
...
public class EventProcessor {
    ...
    @Action
    public Result handleEventA(
            @Trigger EventA eventA,  // <1>
            EventB eventB
    ) {
    ...
    @Action
    public Result handleEventB(
            EventA eventA,
            @Trigger EventB eventB  // <2>
    ) {
...
NOTE: `@Trigger` checks that the parameter matches the `lastResult()` on the blackboard.
