...
- A Lucene store exposes vector search, text search, AND result expansion tools
...
`SearchOperations` is the tag interface for search functionality. Concrete implementations implement one or more subinterfaces:
...
- **TextSearchTools**: `textSearch(query, topK, threshold)` - BM25 full-text search, `regexSearch(regex, topK)`
...
To implement a custom RAG store, implement as many of the `SearchOperations` subinterfaces as your store supports:

[source,kotlin]
----
class MyCustomSearchOperations : VectorSearch, TextSearch {
