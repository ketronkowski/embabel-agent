...
WARNING: State classes should be **static nested classes** (Java) or **top-level classes** (Kotlin), not non-static inner classes.
...
In Kotlin, data classes declared inside a class are inner by default; use top-level declarations or companion object patterns instead.
